...
Why would you ever need to add an additional Build layer, and even add it in different positions in the dependency stack? I wish we could avoid it by all means, but there are cases when we need to build some build tools before we can build the layers using them.
...
* Both the host and plugins need to reference some host interfaces and some plugin interfaces. Therefore, the base plugin interfaces and base host interfaces need to be implemented 
